...
class State:
    ...
    def __str__(self):
        return f"[State] x={self.x:.4f}, y={self.y:.4f}, yaw={self.yaw:.4f}, v={self.v:.4f}, w={self.w:.4f}"
...
class ControlState:
    ...
    def __str__(self):
        if self.control_type == "basic":
            ...
            return f"[ControlState] lw={self.lw}, rw={self.rw}"
        elif self.control_type == "bicycle":
            return f"[ControlState] a={self.a}, delta={self.delta}"
...
def EndPoint(pose, lidar_params, sensor_data, skip_max=False):
    ...
    for i in range(int(lidar_params[0])):
        ...
        pts_list.append(
            [pose[0] + sensor_data[i] * np.cos(np.deg2rad(theta)), pose[1] + sensor_data[i] * np.sin(np.deg2rad(theta))]
        )
    return pts_list
